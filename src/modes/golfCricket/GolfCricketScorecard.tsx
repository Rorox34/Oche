...
import type { GolfCricketPlayer } from '../../core/golfCricket/types';
import { relativeToPar } from '../../core/golfCricket/engine';
import { formatRelativePar } from '../../core/golfCricket/format';
...
function StrokeCell({ score, marks, showMarks }: { score: number; marks: number; showMarks: boolean }) {
  if (score >= 0) {
    const tone =
      score === 1 ? 'text-gold' : score === 2 ? 'text-green' : score === 3 ? 'text-cream' : 'text-red';
    return (
      <span className={`mx-auto font-display text-xl font-bold leading-none tabular-nums ${tone}`}>
        {score}
      </span>
    );
  }
...
export function GolfCricketScorecard({ targets, players, currentPlayer, round }: Props) {
  ...
  // pas encore fermé le même nombre de trous), mis en valeur s'il y a un écart.
          ...
          return (
            <Fragment key={i}>
              ...
              {players.map((p, pi) => (
                ...
                >
                  <StrokeCell score={p.holeScores[i]} marks={p.holeMarks} showMarks={onHole} />
                </span>
              ))}
            </Fragment>
          );
